...
| r_account_menu | researcher | top-right name → menu | screens/r_account_menu.png · hierarchy/r_account_menu.json |  |
...
| a_person_e2e_self_reload | admin | hard reload on /people/<E2E id> | screens/a_person_e2e_self_reload.png · hierarchy/a_person_e2e_self_reload.json | verification of F-person-routing |
| a_person_hash_switch | admin | hash change to /people/<Celia id> without reload | screens/a_person_hash_switch.png · hierarchy/a_person_hash_switch.json | still shows E2E Bot |
| state_error_backend_down_26s | admin | /people with supabase blocked, +26 s | screens/state_error_backend_down_26s.png · hierarchy/state_error_backend_down_26s.json | ST-ERROR evidence after timeout |
| state_error_dashboard_down_26s | admin | /app/dashboard with supabase blocked, +26 s | screens/state_error_dashboard_down_26s.png · hierarchy/state_error_dashboard_down_26s.json | ST-ERROR evidence after timeout |

## Flow-suite evidence

Driven by Playwright (see `flows-results.json`; Maestro's Chromium driver hung on this machine, three attempts). The two flows that write to the live database were cleaned up afterwards:
...
update people set profile_status='draft' where email='andre.berloga+e2e@gmail.com';
```
...
| flow_add_output_5_FAIL | flow `add_output` | step evidence: FAIL | screens/flow_add_output_5_FAIL.png | flow suite |
...
| flow_review_queue_4_after_reject | flow `review_queue` | step evidence: after reject | screens/flow_review_queue_4_after_reject.png | flow suite |
...
- Destructive confirm on a *single* Reject: none exists to capture (finding).
